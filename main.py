...
def add_item_to_notion_database(game, achievements_info, review_text, steam_store_data):
    ...
    if PROPERTY_TYPES.get(PROPERTY_MAPPING["TAGS"], "") == "checkbox":
        ...
        tags = [] 9 for tag in steam_store_data.get('tag', []): 10     if isinstance(tag, dict): 11         # 如果标签已经是字典格式，确保它有 'name' 字段 12         if 'name' in tag: 13             tags.append({"name": tag['name']}) 14         else: 15             # 处理没有 'name' 字段的情况 16             logger.warning(f"无效标签格式: {tag}") 17     else: 18         # 如果标签是字符串，直接使用 19         tags.append({"name": str(tag)})
        properties[PROPERTY_MAPPING["TAGS"]] = {
            "type": "multi_select",
            "multi_select": tags
        }
